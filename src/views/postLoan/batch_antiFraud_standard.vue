  ...
  <div>
    <div class="container">
      <div class="white-bg">
        ...
        <h2 class="text-center">类型：贷后风险监测</h2>
        <div class="form-horizontal">
          <div class="form-group">
            <label class="col-md-4 control-label">请下载模版：</label>
            <div class="col-md-5">
              <router-link to="/api/template/贷后风险监测.xlsx" target="_blank" class="el-button el-button--danger">下载模版</router-link>
    ...
    methods: {
      ...
      changePrices(response){
        ...
        if(response.success && response.data!=null){
          _this.price = true
          _this.cost = new Number(response.data.cost).toFixed(2)
      ...
      searchResult(){
        ...
        _this.$store.dispatch("CREDITRISK_BQUERY",{ param }).then((res,req)=>{
          if(res.success){
            sessionStorage.setItem('_import_tradeId',res.data)
            _this.$router.push({path: "batch_result"})
          }
          _this.fullscreenLoading = false
          _this.$notify({
            title: '提示信息',
